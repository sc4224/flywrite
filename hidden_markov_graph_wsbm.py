...
def nb_dispersion(raw_dispersion, eps=1e-4):
    """Global dispersion parameter r > 0. Increased eps to prevent lgamma explosion."""
...
def wsbm_objective_dense(
    ...
    Negative Binomial parameterization:
        Y ~ NB(mean=mu, dispersion=r)
    """
    mu = nb_mean(U_left, U_right, bias).to(dtype=CC.dtype, device=CC.device)
...
for task_id in range(0, 10):
    ...
    optimizer = torch.optim.AdamW([U_left, U_right, bias, raw_dispersion, q_logits], lr=0.01)
    ...
    print("Results saved: 'cluster_assignments.npy', 'U_left.npy', 'U_right.npy', 'cluster_assignment_dict.json', 'bias.npy', 'dispersion.npy'")

    del U_left, U_right, bias, raw_dispersion, q_logits, optimizer, cluster_assignments, cluster_scores, U_left_final, U_right_final, bias_final, dispersion_final
    gc.collect()
    if device == 'cuda':
