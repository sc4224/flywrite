...
space = [
    Integer(32, 64, name='n_components'),
    Integer(10, 1024, name='k'),
...
def objective(**params):
   ...
   # )
    ...
    sim_matrix = cosine_similarity(X, cluster_centers_original)
    labels_cosine = np.argmax(sim_matrix, axis=1)

    score = silhouette_score(X, labels_cosine, metric="cosine")

    del labels, U_orth, cluster_centers_original, labels_np, labels_cosine, sim_matrix, X
    gc.collect()
    if torch.backends.mps.is_available() and torch.device('mps') == torch.device('mps'):
        torch.mps.empty_cache()
...
#    cumulative_means, cumulative_lower, cumulative_upper = [], [], []
    ...
    for batch in range(n_batches):
        _ = Parallel(n_jobs=batch_size)(
            delayed(run_pca_kmeans)(
                batch, index + 1, device=gpu_list[index % gpu_count]
...
#    print("  optimizer:", None if res_gp is None else res_gp.x[4])
    with open("best_hyperparams_results.txt", "w") as f:
        print("Best min distance: {:.4f}".format(-res_gp.fun), file=f)
        print("Best hyperparameters:", file=f)
        print("  n_components:", res_gp.x[0], file=f)
        print("  k (clusters):", res_gp.x[1], file=f)
        ...
        print("Best Silhouette Score:", -res_gp.fun, file=f)
